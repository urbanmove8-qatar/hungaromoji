...
(function() {
    ...
    /**
     * Finds the correct URL for an emoji by looking it up in the static map.
    ...
    // 1. Sort keys by length in descending order to handle multi-character/codepoint emojis first.
    // 2. Escape the keys for use in a Regular Expression.
    // 3. Create the final regex pattern, capturing the match for lookup.
    const sortedEmojiKeys = Object.keys(emojiMap).sort((a, b) => b.length - a.length);
    const escapedKeys = sortedEmojiKeys.map(key => key.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'));
    ...
    function replaceInTextNode(textNode) {
        ...
        // Only proceed if the text content contains any of our emoji patterns
        if (emojiRegex.test(text)) {
            // Reset regex pointer to start of string for exec loop
            emojiRegex.lastIndex = 0; 
            
            let lastIndex = 0;
            // Use a DocumentFragment for performance: assemble all nodes here before a single DOM injection
