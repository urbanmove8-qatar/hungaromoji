...
(function() {
    ...
    // 1. Get all the keys from the map.
    const keys = Object.keys(emojiMap);
    
    // 2. Sort keys by length in descending order. This is the critical fix:
    // It ensures multi-codepoint emojis like "🏳️‍🌈" (5 characters) are matched
    // before their components like "🏳️" (3 characters).
    const sortedKeys = keys.sort((a, b) => b.length - a.length);

    // 3. Escape the keys for use in a Regular Expression (for keys like "penguinmod:joe" or special regex characters).
    // This is vital for keys that contain special regex characters (like $^*.+?).
    const escapedKeys = sortedKeys.map(key => key.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'));

    // 4. Create the final regex pattern.
    ...
    function replaceInTextNode(textNode) {
        ...
        // Use the pre-calculated and sorted/escaped pattern
